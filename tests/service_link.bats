...
@test "($PLUGIN_COMMAND_PREFIX:link) error when the service is already linked to app" {
  ...
  echo "output: $output"
  echo "status: $status"
  assert_contains "${lines[*]}" "Already linked as GOTENBERG_URL"
  assert_failure

  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) exports GOTENBERG_URL to app" {
  run dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  echo "output: $output"
  echo "status: $status"
  url=$(dokku config:get my-app GOTENBERG_URL)
  assert_contains "$url" "dokku-gotenberg-ls:11211"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when GOTENBERG_URL already in use" {
  dokku config:set my-app GOTENBERG_URL=host:11211
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_GOTENBERG_AQUA_URL"
  assert_success

  dokku "$PLUGIN_COMMAND_PREFIX:link" ms my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_GOTENBERG_BLACK_URL"
  assert_success
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.gotenberg.ls:dokku-gotenberg-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps GOTENBERG_DATABASE_SCHEME variable" {
  dokku config:set my-app GOTENBERG_DATABASE_SCHEME=gotenberg2
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app GOTENBERG_URL)
  assert_contains "$url" "gotenberg2://dokku-gotenberg-ls:11211"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
